...
function Wald0Index{
    [CmdletBinding()]
    Param(
        [Parameter(ValueFromPipeline=1,ValueFromPipelineByPropertyName=1,Position=0)][Alias('TargetGroup')][String]$Name = "DOMAIN ADMINS@EXTERNAL.LOCAL"
        )
    Begin{
        $Result = @()
        }
...
.EXAMPLE
   Edge -MemberOfGroup CONTRACTINGF@INTERNAL.LOCAL -Return Users -Degree *
...
function BloodHound-EdgeReverse{
	...
	Process{
        foreach($Target in $DynTarget.Value){
            ...
            # Call Dog
            if(-Not$DynCypher.IsSet){$Result += DogPost $Query -Params $Params -Raw}
            }}
	...
	End{
		...
		else{Return ($Result | Select -expand data).data |Sort -Property name -Unique}
		}}
#####End
...
###########################################################
##################################### BloodHound-EdgeCustom <----------------------- /!\ FIX!!

<#
.Synopsis
   Get Node by Custom Edge
.DESCRIPTION
   Query Custom Edge RelationShips
.EXAMPLE
   EdgeCustom -ToUser bob -Custom IsSister -Return User
#>
function BloodHound-EdgeCustom{
    ...
    DynamicParam{
        ...
        # DynP
        $Dynname   = dynP -Name 'Name' -Type String -Mandat 1 -Pos 0 -Pipe 1 -PipeProp 1 -VSet $VSet 
        $DynCustom = dynP -Name 'Edge' -Type String[] -Mandat 0 -Pos 2 -Pipe 0 -PipeProp 0 -VSet $Null
    ...
    Begin{
        ...
        if($DynCustom.IsSet){
            $C =':' + $DynCustom.Value -join '|:'
            } 
        # If To
        if($PSN -match '^To'){
            $Query = "MATCH (A:$ReturnType), (B:$TargetType {name: {target}}), p=(A)-[R$C*1..$Max]->(B) RETURN A"
            }
        # If From
        if($PSN -match '^From'){
            $Query = "MATCH (A:$ReturnType), (B:$TargetType {name: {target}}), p=(A)<-[R$C*1..$Max]-(B) RETURN A"
...
.DESCRIPTION
   Create new Edges between 2 existing Nodes
.EXAMPLE
    EdgeCreate -UserToUser -CustomEdge IsSister -From alice -To bob
    /!\ Edges have a direction
.EXAMPLE <-------------------------------------------------------------------------------- ?? Edge Source better ??
   'jane','alice' | EdgeCreate -UserToUser -CustomEdge IsBrother -From bob -Verbose
   Accepts Multiple Edge targets over pipeline
#>
...
.EXAMPLE
    EdgeDelete -UserToUser -CustomEdge IsBrother -From bob -to jane
#>
...
.Synopsis
   Bloodhound Path Via
.DESCRIPTION
   Retrieve Bloodhound Path Via specific Node
.EXAMPLE
...
function BloodHound-PathVia{
    ...
    DynamicParam{
        ...
        $DynTo            = DynP -Name 'To'            -Type String     -Mandat 1 -Pos 2 -Pipe 0 -PipeProp 0 -VSet $VTo
        $DynVia           = DynP -Name 'ViaName'       -Type String     -Mandat 1 -Pos 3 -Pipe 1 -PipeProp 1 -VSet $VVia
...
<#
.Synopsis
   Generate BH Cypher Queries
   Calc Wald0Index for specified group
.DESCRIPTION
   Basic Cypher Query Generator + Set ClipBoard
   OR Wald0Index calculation
.EXAMPLE
   Cypher -UserToComputer -From ACHAVARIN@EXTERNAL.LOCAL -To NYX.EXTERNAL.LOCAL -PathType AllShortest -MaxHop 7
.EXAMPLE
   Cypher -Union -QueryA $Q1 -QueryB $Q2
.EXAMPLE
   Cypher -Union -QueryCollection @($Q1,$Q2,$Q3,$Q4)
.EXAMPLE
   Cypher -Wald0Index 'AUDIT_A@EXTERNAL.LOCAL'
   Calc Wald0Index
.EXAMPLE
   'AUDIT_A@EXTERNAL.LOCAL','AUDIT_B@EXTERNAL.LOCAL','AUDIT_C@EXTERNAL.LOCAL'| Cypher -Wald0Index |ft
   Accepts Multiple Group names over pipeline 
#>
function BloodHound-PathCypher{
    [CmdletBinding(DefaultParameterSetName='Wald0Index')]
    ...
    Process{
        ...
        Elseif($PSCmdlet.ParameterSetName -match 'Union'){
            # from Collection
            if($QueryCollection){               
                $Result= $QueryCollection -join "`r`nUNION`r`n"
                }
            # Two Queries only
            else{               
                $Result = "$QueryA`r`nUNION`r`n$QueryB"
                # Set Clipboard
...
function BloodHound-Help{
    ...
    ################################################################################################################
    @{Cmdlet='BloodHound-Node'        ; Synopsis='Get Node by Name'                       ; Help='Help Node'       }
    @{Cmdlet='BloodHound-NodeSearch'  ; Synopsis='Search Node by Key|Prop|PropValue'      ; Help='Help NodeSearch' }
    @{Cmdlet='BloodHound-NodeCreate'  ; Synopsis='Add Node to DB'                         ; Help='Help NodeCreate' }
    @{Cmdlet='BloodHound-NodeUpdate'  ; Synopsis='Add/Update Node Properties'             ; Help='Help NodeUpdate' }
    @{Cmdlet='BloodHound-NodeDelete'  ; Synopsis='Remove Node from DB'                    ; Help='Help NodeDelete' }
    @{Cmdlet='BloodHound-Edge      '  ; Synopsis='Get Node by Edge'                       ; Help='Help Edge'       }
    @{Cmdlet='BloodHound-EdgeReverse' ; Synopsis='Get Node by Reverse Edge'               ; Help='Help EdgeReverse'}
    @{Cmdlet='BloodHound-EdgeCustom'  ; Synopsis='Get Node by Custom Edge'                ; Help='Help EdgeCustom' }
    @{Cmdlet='BloodHound-EdgeCreate'  ; Synopsis='Create Edge between Nodes'              ; Help='Help EdgeCreate' }
    @{Cmdlet='BloodHound-EdgeDelete'  ; Synopsis='Remove Edge between Nodes'              ; Help='Help EdgeDelete' }
    @{Cmdlet='BloodHound-Path'        ; Synopsis='Shortest Path - Node to Node'           ; Help='Help Path'       }
    @{Cmdlet='BloodHound-PathVia'     ; Synopsis='Shortest Path - Node to Node via Node'  ; Help='Help PathVia'    }
    @{Cmdlet='BloodHound-PathCypher'  ; Synopsis='Generate Cypher | Wald0Index'           ; Help='Help Cypher'     }
    ################################################################################################################
    )
    # Return Help Obj
