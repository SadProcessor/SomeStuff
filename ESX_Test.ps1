...
## PULL REQUESTS ?
# /?\ Stager OutFile     <------------------------- /?\ Probably cool (w\ static) 
#                                                       PR Branch [SadProcessor-StagerOutfile]
...
function EmpireCall{
    ...
    Switch($Act){
      ...
      ##Agent ##############################################################################################
        ...
        AgentUploadFile  {$M='POST'  ; $K=$Null            ; $R="/api/agents/$Spec/upload"       } <# OK #> #<-------- /!\ Check/Fix
        AgentDownloadFile{$M='POST'  ; $K=$Null            ; $R="/api/agents/$Spec/download"     } <# -- #> #<-------- /!\ Check how it works /Fix
      ...
      ##Cred ###############################################################################################
        ...
        CredAdd          {$M='POST'  ; $K=$Null            ; $R="/api/creds"                     } <# OK #> #<-------- /!\ Check required POST format
      ##Listener ###########################################################################################
...
Function Empire-Cred{
    ...
    Param(
        ...
        # Password to Add
		[Parameter(Mandatory=1,ParameterSetName='CredAdd',Position=0)][Alias('Type')][String]$domain,
		# UserName to Add
		[Parameter(Mandatory=1,ParameterSetName='CredAdd',Position=1)][Alias('Name','Usr')][String]$username,
        # Password to Add
		[Parameter(Mandatory=1,ParameterSetName='CredAdd',Position=2)][Alias('Pwd')][String]$password,
        # Password to Add
		[Parameter(Mandatory=0,ParameterSetName='CredAdd',Position=3)][String]$notes='',
        # Password to Add
		[Parameter(Mandatory=0,ParameterSetName='CredAdd',Position=4)][String]$credtype='',
        # Password to Add
		[Parameter(Mandatory=0,ParameterSetName='CredAdd',Position=5)][String]$sid=''
		)
    DynamicParam{
        # Filter ParamSet
