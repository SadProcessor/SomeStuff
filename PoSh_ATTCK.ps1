...
function Get-ATTCKnowledge{
    ...
    if($PSCmdlet.ParameterSetName -eq 'Sync'){
        ...
        Write-Verbose "[+] Getting Json Object from Github..."
        ...
        if($PSVersionTable.PSEdition -eq 'Core'){
            $Json = curl $url | ConvertFrom-Json 
            }
        # Stop if not found
        ...
        Write-Verbose "[+] Sorting Group Objects..."
        ...
        Write-Verbose "[+] Sorting Technique Objects..."
        ...
        Write-Verbose "[+] Sorting Software Objects..."
    ...
    Else{
        ...
        # Else Cmdlet Memo
            ...
            $Banner
            [PSCustomObject]@{Cmdlet='Get-ATTCKnowledge' ;Alias='ATTCKnowledge' ;Help='Help ATTCKnowledge' }
            [PSCustomObject]@{Cmdlet='Get-ATTCKTechnique';Alias='ATTCKTechnique';Help='Help ATTCKTechnique'}
            [PSCustomObject]@{Cmdlet='Get-ATTCKGroup'    ;Alias='ATTCKGroup'    ;Help='Help ATTCKGroup'    }
            [PSCustomObject]@{Cmdlet='Get-ATTCKSoftware' ;Alias='ATTCKSoftware' ;Help='Help ATTCKSoftware' }
            }}}
#########End
...
function Get-ATTCKTechnique{
    ...
    Param(
        ...
        # Apply Filters
        [Parameter(Mandatory=1,ParameterSetName='Filter')][Switch]$Filter,
        # Match Description
...
.DESCRIPTION
   ...
   ## Syntax 
   ATTCKGroup [-Online]
   ATTCKGroup [-name] [-Value] <tab> [-Online] [-Reference]
   ATTCKGroup -ID [-Value] <tab> [-Online] [-Reference]
   ATTCKGroup -Description [-Match] <input>

   See examples for details 
   PS> Help ATTCKGroup -Example
.EXAMPLE
    ATTCKGroup
...
.EXAMPLE
    ...
    Add -Reference -Online -> open reference in Browser
.EXAMPLE
    ATTCKGroup -ID G0023 | Select Description
...
$all = irm https://raw.githubusercontent.com/mitre/cti/master/enterprise-attack/enterprise-attack.json

$All.objects | select type | sort type -Unique
$All.objects | where type -eq attack-pattern
$All.objects | where type -eq identity

# Techniques (& Mitigation) Props
$All.objects | where type -eq attack-pattern |% {$_|gm|? MemberType -eq Noteproperty | select name} | sort name -Unique
$All.objects | where type -eq course-of-action |% {$_|gm|? MemberType -eq Noteproperty | select name} | sort name -Unique
# Groups Props
$All.objects | where type -eq intrusion-set |% {$_|gm|? MemberType -eq Noteproperty | select name} | sort name -Unique
# Software Props
$All.objects | where type -eq malware |% {$_|gm|? MemberType -eq Noteproperty | select name} | sort name -Unique
$All.objects | where type -eq tool |% {$_|gm|? MemberType -eq Noteproperty | select name} | sort name -Unique
